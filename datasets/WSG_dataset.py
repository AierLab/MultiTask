...
def read_txt(txt_name = 'RealHaze.txt',sample_num=5000):
    ...
    for path in paths_random:
        path_in.append(path.split('/')[-1])
        path_gt.append(path.split('/')[-1])
    return path_in,path_gt

class my_dataset(Dataset):
    def __init__(self, rootA, rootA_label,rootA_txt, rootB, rootB_label, rootB_txt,rootC, rootC_label,rootC_txt, crop_size =256,
        ...
        self.imgs_gt_A = [rootA_label + k for k in gt_files_A]#gt_imgs  os.path.join(rootA_label
        ...
        self.imgs_gt_B = [rootB_label + k for k in gt_files_B] # gt_imgs  name keep same
       ...
       # in_files_C = os.listdir(rootC_in)
        ...
        self.imgs_gt_C = [rootC_label + k for k in gt_files_C] #[os.path.join(rootC_label, k) for k in in_files_C]  # gt_imgs  name keep same
